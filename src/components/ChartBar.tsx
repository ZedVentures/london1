...
const ChartBar: React.FC<ChartBarProps> = ({ 
  ...
  return (
    <div 
      ref={barRef}
      className={`flex flex-col items-center flex-1 mx-0.5 sm:mx-1 transition-all duration-1000 ${
    ...
    >
      <div className="relative flex flex-col items-center justify-end h-32 sm:h-40 md:h-48 lg:h-56 w-full">
        ...
        >
          ...
          <div className={`absolute inset-0 flex items-center justify-center transition-all duration-500 ${
            ...
            <div className="text-white font-bold text-xs sm:text-sm text-center leading-tight transform -rotate-0">
              <div className="bg-gray-900/95 backdrop-blur-sm px-1.5 sm:px-2 lg:px-3 py-1 sm:py-1.5 lg:py-2 rounded-md sm:rounded-lg shadow-lg border border-white/20">
          ...
          {data.growth && (
            <div className={`absolute -top-12 left-1/2 transform -translate-x-1/2 text-xs font-bold whitespace-nowrap transition-all duration-500 ${formatGrowthColor(data.growth)} ${
              ...
              <div className="bg-white/95 backdrop-blur-sm px-1 sm:px-2 py-0.5 sm:py-1 rounded-sm sm:rounded-md shadow-md border border-gray-200 text-xs">
      ...
      <div className={`mt-1 sm:mt-2 text-center transition-all duration-500 ${
        ...
        <div className="bg-white/90 backdrop-blur-sm px-2 sm:px-3 py-0.5 sm:py-1 rounded-md sm:rounded-lg shadow-sm border border-gray-200">
          <div className="text-xs sm:text-sm font-bold text-purple-800">{data.year}</div>
        </div>
      </div>
    </div>
  );
